...
item1 = Item(name="African Championship", description = "Standard Bank Sponsors the IRONMAN African Championship to be hosted on April 15 on Nelson Mandela Bay, South Africa", category = category1)
session.add(item1)
session.commit()

item2 = Item(name="Subic Bay Philippines", description = "Century Tuna Sponsors the Subic Bay Philippines Ironamn on June 3 in Subic Bay Philipines", category = category1)
session.add(item2)
session.commit()

item3 = Item(name="70.3 Dubai", description = "Dubai will host an Ironman 70.3 on February 2 in Dubai, United Arab Emirates", category = category2)
session.add(item3)
session.commit()

item4 = Item(name="70.3 Geelong", description = "Australia will have an Ironman 70.3 on February 18 in Geelong, Victoria", category = category2)
session.add(item4)
session.commit()

item5 = Item(name="Arizona Super and Sprint Weekend", description = "February 10 will have a Super and Sprint Spartan Race in Arizona", category = category3)
session.add(item5)
session.commit()

item6 = Item(name="South Florida Sprint Weekend", description = "The Sprint may be our shortest distance race, but it is still a favorite amongst both new and returning racers. It is the perfect distance for those looking to start their Spartan journey.", category = category3)
session.add(item6)
session.commit()
